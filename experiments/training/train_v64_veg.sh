...
. $BASE/scripts/common/gpu_wait.sh
...
$CE python3 -m carla2real.evaluation.veg_report "$NAME" >> "$LOG" 2>&1
$CE python3 -m carla2real.evaluation.tail_check "$NAME" "$PHS" >> "$LOG" 2>&1
$CE python3 -m carla2real.evaluation.road_sky_ceiling >> "$LOG" 2>&1
$CE python3 -m carla2real.evaluation.epoch_sweep \
