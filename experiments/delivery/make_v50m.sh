...
for T in $TOWNS; do
  ...
  if ! $CE python3 -m carla2real.validation.check_reference "$SRC" "$REC/rgb" >> "$LOG" 2>&1; then
    echo "  $T: REFERENCE MISMATCH -- skipping" >> "$LOG"; continue
  fi

  $CE python3 -u -m carla2real.postprocessing.protect_vehicle_colour "$SRC" "$REC/rgb" \
  ...
  $CE python3 -u -m carla2real.postprocessing.protect_buildings "$A" "$REC/rgb" \
...
w.release(); print('  sharpened %d frames'%n)" >> "$LOG" 2>&1
  ...
  BLDG_STRENGTH=0.35 ROAD_STRENGTH=0.5 CAR_STRENGTH=${CAR_S:-0.55} CAR_FLOW=${CAR_F:-4.5} WINDOW=3 \
    $CE python3 -u -m carla2real.postprocessing.class_deshimmer "$B" "$DRR/${PHS}_label" "$OUT/${T}_ds.avi" --alt 1.0 >> "$LOG" 2>&1
  ...
  if [ -s "$SRC63" ]; then
    ROBUST=1 SLEW=0.08 $CE python3 -u -m carla2real.postprocessing.fuse_colour "$C" "$SRC63" "$OUT/${T}_fuse.avi" 1.0 91 \
      >> "$LOG" 2>&1
...
w.release()" >> "$LOG" 2>&1
  ...
  [ -s "$G" ] && $CE python3 -m carla2real.evaluation.score_vp "$G" "$DEST/logs_${TAG}/${T}_sunny.log" \
...
$CE python3 -m carla2real.evaluation.flicker_report v50l v50m --frames 300 >> "$LOG" 2>&1
bash $BASE/scripts/delivery/organise_calibrated.sh >> "$LOG" 2>&1
