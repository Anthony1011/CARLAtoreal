...
w.release()" >> "$LOG" 2>&1
  ...
  [ -s "$G" ] && $CE python3 -m carla2real.evaluation.score_vp "$G" "$DEST/logs_v50j/${T}_sunny.log" \
      --json "$DEST/logs_v50j/${T}_sunny_score.json" >> "$LOG" 2>&1
  rm -f "$OUT/${T}_col.avi" "$OUT/${T}_bld.avi" "$OUT/${T}_sharp.avi" "$OUT/${T}_ds.avi"
  echo "  $T DONE -> $DN" >> "$LOG"
done

$CE python3 -m carla2real.evaluation.flicker_report v50d v50j --frames 300 >> "$LOG" 2>&1
