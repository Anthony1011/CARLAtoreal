...
for T in town03 town04 town05 town06 town10hd; do
  ...
  $CE python3 -u -m carla2real.postprocessing.temporal_deshimmer "$SRC" "$OUT/${T}_ds.avi" \
...
w.release()" >> "$LOG" 2>&1
  ...
  [ -s "$G" ] && $CE python3 -m carla2real.evaluation.score_vp "$G" "$DEST/logs_v50d/${T}_sunny.log" \
