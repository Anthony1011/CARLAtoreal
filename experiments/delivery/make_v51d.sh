...
for T in town03 town04 town05 town06 town10hd; do
  ...
  $CE python3 -u -m carla2real.postprocessing.temporal_deshimmer "$SRC" "$OUT/${T}_ds.avi" --alt 1.0 --flow 6.0 >> "$LOG" 2>&1
  A=$OUT/${T}_ds.avi; [ -s "$A" ] || A=$SRC
  $CE python3 -u -m carla2real.postprocessing.despeckle_night "$A" "$OUT/${T}_dsp.avi" >> "$LOG" 2>&1
...
w.release()" >> "$LOG" 2>&1
  ...
  [ -s "$G" ] && $CE python3 -m carla2real.evaluation.score_vp "$G" "$DEST/logs_v51d/${T}_night.log" \
      --json "$DEST/logs_v51d/${T}_night_score.json" >> "$LOG" 2>&1
  rm -f "$OUT/${T}_ds.avi" "$OUT/${T}_dsp.avi"
  echo "  $T DONE -> $DN" >> "$LOG"
done

$CE python3 -m carla2real.evaluation.flicker_report v51 v51d --weather night --frames 300 >> "$LOG" 2>&1
