...
for T in "$@"; do
  ...
  if ! $CE python3 -m carla2real.validation.check_reference "$DD" "$REC/rgb" >> "$LOG" 2>&1; then
  ...
  $CE python3 -u -m carla2real.postprocessing.stabilize_frames_v2 --frames_dir "$DD" --out "${B}_baseline.avi" \
...
for i,f in enumerate(sorted(glob.glob('$DD/*_synthesized_image.jpg'))): shutil.copy(f,'$PROC/%05d.jpg'%i)" >> "$LOG" 2>&1
  ...
  $CE python3 -m carla2real.postprocessing.make_v33 "${B}_baseline.avi" "$OD" "$B" "VP" >> "$LOG" 2>&1
  [ -s "${B}_v33_sunny.avi" ] && S=${B}_v33_sunny.avi
  rm -rf "$PROC" "$OD"
  fi

  GRAIN_SCALE=0 BLOOM_SCALE=0 CHROMA_SCALE=0 \
    $CE python3 -m carla2real.postprocessing.photoreal_post "$S" "${B}_photoreal.avi" 0.7 >> "$LOG" 2>&1
  ...
  if [ "$W" = night ]; then
    $CE python3 -u -m carla2real.postprocessing.protect_light_pools "$P" "$REC/rgb" "$DRR/${PHS}_label" \
      "${B}_pools.avi" 0.8 18 >> "$LOG" 2>&1
    [ -s "${B}_pools.avi" ] && P=${B}_pools.avi
  fi
  $CE python3 -u -m carla2real.postprocessing.protect_lane_markings "$P" "$REC/rgb" "$DRR/${PHS}_label" "${B}_lane.avi" 45 0.9 >> "$LOG" 2>&1
  C=${B}_lane.avi; [ -s "$C" ] || C=$P
  $CE python3 -u -m carla2real.postprocessing.protect_billboards "$C" "$REC/rgb" "$DRR/${PHS}_label_rich" "${B}_bb.avi" 0.85 3 >> "$LOG" 2>&1
  C2=${B}_bb.avi; [ -s "$C2" ] || C2=$C
  TL_CLEAN=1 $CE python3 -u -m carla2real.postprocessing.protect_traffic_lights_carla "$C2" "$REC/rgb" "$DRR/${PHS}_label" \
...
w.release()" >> "$LOG" 2>&1
  ...
  [ -s "$G" ] && $CE python3 -m carla2real.evaluation.score_vp "$G" "$DEST/logs_${TAG}/${low}_${W}.log" \
...
bash $BASE/scripts/delivery/refresh_new.sh >> "$LOG" 2>&1
